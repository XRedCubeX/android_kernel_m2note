...
#ifdef MEIZU_M81
static int temperature_debug = false;
INT32	temperature_set = 0;
int battery_charger_dis = 0;
#endif
...
static void ac_update(struct ac_data *ac_data)
{
    ...
    if( BMT_status.charger_exist == KAL_TRUE )
    {
		...
		(BMT_status.charger_type == MEIZU_2_0A_CHARGER) ||
		(BMT_status.charger_type == APPLE_CHARGER) ||
		(BMT_status.charger_type == U200_CHARGER))
        {
            ac_data->AC_ONLINE = 1;
...
#ifdef MEIZU_M81
	/* add charging temperature 10oC, between 2-12oC set charging current 0.3C*/
	if (BMT_status.temperature <= MED_CHARGE_TEMPERATURE)
	{
		if (g_batt_temp_status == TEMP_POS_MEDIUM)
		{
		    return PMU_STATUS_OK;
		}
		printk("[BATTERY] Battery Under Temperature 10oC!!\n\r");
		g_batt_temp_status = TEMP_POS_MEDIUM;
		//BMT_status.batt_health = POWER_SUPPLY_HEALTH_GOOD;
		BMT_status.bat_charging_state = CHR_PRE;
		return PMU_STATUS_OK;
	}
	else if (g_batt_temp_status == TEMP_POS_MEDIUM)
	{
		if ((BMT_status.temperature > MED_CHARGE_TEMPERATURE) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE))
		{
			printk("[BATTERY] Battery Temperature raise from %d to %d, charging current rising 0.5C!!\n\r",
				MED_CHARGE_TEMPERATURE,BMT_status.temperature);
			g_batt_temp_status = TEMP_POS_NORMAL;
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		}
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		if (previous_charger_exist == KAL_FALSE) {
			...
			if (BMT_status.charger_type == STANDARD_CHARGER || DISO_data.diso_state.cur_vdc_state == DISO_ONLINE){
				...
				data = MEIZU_2_0A_CHARGER_CURRENT / 100;		//mA
		#endif
			else	// USB
				 data = USB_CHARGER_CURRENT / 100;		//mA
		#endif	/* endif MEIZU_M81*/
...
void mt_battery_GetBatteryData(void)
{
	...
	else
	{
                ...
                if (temperature_debug) {
                    BMT_status.temperature = temperature_set;
                } else {
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		mt_battery_CheckBatteryStatus();
                ...
                if(battery_charger_dis)
                {
                    ...
                    charger_enable = KAL_FALSE;
                    battery_charging_control(CHARGING_CMD_ENABLE,&charger_enable);
                }
                else
...
#ifdef MEIZU_M81
static ssize_t show_Battery_Charger_Disable(struct device *dev,struct device_attribute *attr, char *buf)
{
    battery_charging_control(CHARGING_CMD_DUMP_REGISTER, NULL);
    return sprintf(buf, "%u\n", battery_charger_dis);
}

static ssize_t store_Battery_Charger_Disable(struct device *dev,struct device_attribute *attr,
		const char *buf, size_t size)
{
    if(buf != NULL && size != 0)
    {
        battery_charger_dis = simple_strtol(buf, NULL,10);
        if(!battery_charger_dis)
        {
...
static DEVICE_ATTR(Battery_Charger_Disable, 0664, show_Battery_Charger_Disable, store_Battery_Charger_Disable);
#endif
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // platform_driver API */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
...
#ifdef MEIZU_M81
           ret_device_file = device_create_file(&(dev->dev), &dev_attr_Battery_Charger_Disable);
#endif
